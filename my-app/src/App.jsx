  ...
  if(item.kind==='VIDEO'){
    return(
      <div style={{position:'relative',maxWidth:'90vw',maxHeight:'90vh',display:'flex',alignItems:'center',justifyContent:'center'}}
        ...
        <video src={item.url} controls autoPlay style={{maxWidth:'90vw',maxHeight:'90vh',outline:'none'}}/>
      </div>
    );
  }
  const thumbSrc=item.url.replace(/\/([^/]+)$/,'/thumbs/$1');
            ...
            {folder.items.some(it=>it.url)?(
              <div style={{display:'grid',gridTemplateColumns:'repeat(4,minmax(0,1fr))',gap:'10px'}}>
                {folder.items.filter(it=>it.url).map((it,i,arr)=>{
                  ...
                  const thumbSrc=isVideo?null:it.url.replace(/\/([^/]+)$/,'/thumbs/$1');
                  return(
                  <div key={i} className="ca-thumb" style={{animation:`ca-thumb-in .3s ${i*40}ms ease both`,cursor:'pointer'}}
                    ...
                    {isVideo
                      ?<video src={it.url} muted playsInline style={{width:'100%',height:'100%',objectFit:'cover',display:'block',position:'absolute',inset:0}}/>
